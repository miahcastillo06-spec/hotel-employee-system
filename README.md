...
database, the Hotel System form with the dropdown populated from the Employee
System's API, a successful reservation submission, and the saved record
inside the `hotel_db` database.
...
![Form filled](screenshots/02-form-filled.jpeg)

### 3. Reservation created successfully
![Reservation success](screenshots/03-reservation-success.jpeg)

### 4. `hotel_db` → `reservations` table — saved record proving the Create/Store function works
![Reservations table](screenshots/04-reservations-table.png)

### 5. `employee_db` → `employees` table — showing IDs starting at 1001
